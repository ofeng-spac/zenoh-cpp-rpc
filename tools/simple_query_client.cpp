#include <zenoh.hxx>
#include <condition_variable>
#include <iostream>
#include <mutex>
#include <chrono>
#include <thread>
#include <string>

using namespace zenoh;
using namespace std::chrono_literals;

int main(int argc, char **argv) {
    Config config = Config::create_default();
    if (argc > 1) {
        config = Config::from_file(argv[1]);
    } else {
        // 配置为client模式，连接到zenohd路由器
        config.insert_json5("mode", "\"client\"");
        config.insert_json5("connect/endpoints", "[\"tcp/127.0.0.1:7449\"]");
        std::cout << "使用默认配置：连接到 zenohd 路由器 127.0.0.1:7449" << std::endl;
    }

    std::cout << "正在打开会话..." << std::endl;
    auto session = Session::open(std::move(config));

    std::cout << "Query客户端已启动" << std::endl;
    std::cout << "可用命令:" << std::endl;
    std::cout << "  1 - 发送默认查询" << std::endl;
    std::cout << "  2 - 发送Echo查询" << std::endl;
    std::cout << "  3 - 发送时间查询" << std::endl;
    std::cout << "  q - 退出" << std::endl;
    
    while (true) {
        std::cout << "\n请输入命令 (1/2/3/q): ";
        std::string input;
        std::getline(std::cin, input);
        
        if (input == "q" || input == "Q") {
            std::cout << "退出客户端..." << std::endl;
            break;
        }
        
        std::string keyexpr;
        std::string parameters;
        std::string payload;
        
        if (input == "1") {
            keyexpr = "rpc/example/queryable";
            parameters = "";
            std::cout << "发送默认查询..." << std::endl;
        } else if (input == "2") {
            keyexpr = "rpc/example/echo";
            parameters = "";
            payload = "Hello from RPC client!";
            std::cout << "发送Echo查询..." << std::endl;
        } else if (input == "3") {
            keyexpr = "rpc/example/time";
            parameters = "";
            std::cout << "发送时间查询..." << std::endl;
        } else {
            std::cout << "无效命令，请重试" << std::endl;
            continue;
        }
        
        // 发送查询
        std::mutex m;
        std::condition_variable done_signal;
        bool done = false;
        int reply_count = 0;
        
        auto on_reply = [&reply_count](const Reply &reply) {
            reply_count++;
            if (reply.is_ok()) {
                const auto &sample = reply.get_ok();
                std::cout << "[客户端] 收到响应 #" << reply_count << " ('" 
                          << sample.get_keyexpr().as_string_view() << "' : '" 
                          << sample.get_payload().as_string() << "')" << std::endl;
            } else {
                std::cout << "[客户端] 收到错误: " << reply.get_err().get_payload().as_string() << std::endl;
            }
        };
        
        auto on_done = [&m, &done, &done_signal]() {
            std::lock_guard lock(m);
            done = true;
            done_signal.notify_all();
        };
        
        Session::GetOptions options;
        options.target = QueryTarget::Z_QUERY_TARGET_BEST_MATCHING;
        options.timeout_ms = 5000; // 5秒超时
        if (!payload.empty()) {
            options.payload = payload;
        }
        
        std::cout << "发送查询到 '" << keyexpr;
        if (!parameters.empty()) {
            std::cout << "/" << parameters;
        }
        std::cout << "'..." << std::endl;
        session.get(keyexpr, parameters, on_reply, on_done, std::move(options));
        
        // 等待查询完成
        std::unique_lock lock(m);
        if (done_signal.wait_for(lock, 6s, [&done] { return done; })) {
            std::cout << "查询完成，共收到 " << reply_count << " 个响应" << std::endl;
        } else {
            std::cout << "查询超时" << std::endl;
        }
    }
    
    return 0;
}